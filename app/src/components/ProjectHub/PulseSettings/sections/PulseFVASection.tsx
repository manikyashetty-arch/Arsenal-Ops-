...
interface PulseFVASectionProps {
  data: PulseData;
  updateFva: (scope: FvaScope, i: number, patch: Partial<FeatureForecastRow>) => void;
  addFva: (scope: FvaScope) => void;
  removeFva: (scope: FvaScope, i: number) => void;
}
...
const PulseFVASection: React.FC<PulseFVASectionProps> = ({ data, updateFva, addFva, removeFva }) => (
  <Section
              ...
              {rows.map((r, i) => (
                ...
                >
                  <Field label="Feature" className="col-span-5">
                    <TextInput
                      value={r.feature}
                      onChange={(v) => updateFva(scope, i, { feature: v })}
                    />
                  </Field>
                  <Field label="Employee" className="col-span-3">
                    <TextInput
                      value={r.employee}
                      onChange={(v) => updateFva(scope, i, { employee: v })}
                    />
                  </Field>
                  <Field label="Forecast (hrs)" className="col-span-1">
                    <NumberInput value={r.fc} onChange={(n) => updateFva(scope, i, { fc: n })} />
                  </Field>
                  <Field label="Actual (hrs)" className="col-span-2">
                    <NumberInput value={r.act} onChange={(n) => updateFva(scope, i, { act: n })} />
                  </Field>
