...
const TYPE_CONFIG = {
  user_story: { icon: ClipboardList, color: '#E0B954', label: 'Story', bg: 'rgba(224,185,84,0.15)' },
  task: { icon: ClipboardList, color: '#F59E0B', label: 'Task', bg: 'rgba(245,158,11,0.15)' },
  bug: { icon: ClipboardList, color: '#EF4444', label: 'Bug', bg: 'rgba(239,68,68,0.15)' },
  epic: { icon: ClipboardList, color: '#C79E3B', label: 'Epic', bg: 'rgba(199,158,59,0.15)' },
};
...
const AIPlanningModal: React.FC<AIPlanningModalProps> = ({
  ...
  // Reset all state when re-opening (called from handleAIGenerate at parent)
  // State resets happen naturally because the modal unmounts when open=false.
  ...
  const handlePreviewTickets = async () => {
    ...
    try {
      const data = await apiFetch<any>(
        `/api/prd/projects/${projectId}/generate-tickets-preview`,
        {
          method: 'POST',
  ...
  const handleCommitArchitecture = async () => {
    ...
    try {
      const data = await apiFetch<any>(
        `/api/prd/projects/${projectId}/commit-architecture`,
        {
          method: 'POST',
          body: JSON.stringify({
  ...
  const handleCommitRoadmap = async () => {
    ...
    try {
      const data = await apiFetch<any>('/api/roadmap/commit', {
        method: 'POST',
        body: JSON.stringify({ project_id: projectId, parsed_data: roadmapParsedData }),
      });
  ...
  return (
    <>
      ...
      {/* Architecture Editor — rendered outside the scrollable modal body */}
      {editingArchitecture && (
        <ArchitectureEditor
