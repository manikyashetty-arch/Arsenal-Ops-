import { X, Save } from 'lucide-react';
import React from 'react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Modal } from '@/components/ui/modal';

interface EditUserForm {
  name: string;
  email: string;
  github_username: string;
}

interface EditUserModalProps {
  open: boolean;
  onClose: () => void;
  /** Display name of the user being edited (for the title) */
  userLabel: string;
  form: EditUserForm;
  setForm: React.Dispatch<React.SetStateAction<EditUserForm>>;
  onSave: () => void;
  isSaving: boolean;
}

const EditUserModal: React.FC<EditUserModalProps> = ({
  open,
  onClose,
  userLabel,
  form,
  setForm,
  onSave,
  isSaving,
}) => {
  return (
    <Modal open={open} onClose={onClose} maxWidthClass="max-w-md" closeOnBackdrop={!isSaving}>
      <div className="flex items-center justify-between p-5 border-b border-[rgba(255,255,255,0.05)]">
        <div>
          <h2 className="text-lg font-bold text-white">Edit User</h2>
          <p className="text-xs text-[#737373] mt-0.5">{userLabel}</p>
        </div>
        <button
          onClick={onClose}
          disabled={isSaving}
          className="p-2 rounded-lg hover:bg-[rgba(244,246,255,0.05)] text-[#737373] hover:text-white disabled:opacity-50"
        >
          <X className="w-5 h-5" />
        </button>
      </div>
      <div className="p-5 space-y-4">
        <div>
          <label className="text-xs font-medium text-[#737373] block mb-1.5">Name *</label>
          <Input
            value={form.name}
            onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
            placeholder="John Doe"
            className="bg-[rgba(255,255,255,0.025)] border-[rgba(255,255,255,0.07)] text-[#F4F6FF] rounded-xl h-10"
          />
        </div>
        <div>
          <label className="text-xs font-medium text-[#737373] block mb-1.5">Email *</label>
          <Input
            type="email"
            value={form.email}
            onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))}
            placeholder="user@example.com"
            className="bg-[rgba(255,255,255,0.025)] border-[rgba(255,255,255,0.07)] text-[#F4F6FF] rounded-xl h-10"
          />
          <p className="text-[10px] text-[#737373] mt-1">
            Changing the email breaks SSO unless the user&apos;s Google account matches the new
            address.
          </p>
        </div>
        <div>
          <label className="text-xs font-medium text-[#737373] block mb-1.5">GitHub username</label>
          <Input
            value={form.github_username}
            onChange={(e) => setForm((f) => ({ ...f, github_username: e.target.value }))}
            placeholder="octocat"
            className="bg-[rgba(255,255,255,0.025)] border-[rgba(255,255,255,0.07)] text-[#F4F6FF] rounded-xl h-10"
          />
        </div>
      </div>
      <div className="flex justify-end gap-3 p-5 border-t border-[rgba(255,255,255,0.05)]">
        <Button
          variant="ghost"
          onClick={onClose}
          disabled={isSaving}
          className="text-[#737373] rounded-xl px-5"
        >
          Cancel
        </Button>
        <Button
          onClick={onSave}
          disabled={isSaving || !form.name.trim() || !form.email.trim()}
          className="bg-gradient-to-r from-[#E0B954] to-[#B8872A] text-white rounded-xl px-6 font-medium shadow-lg shadow-[#B8872A]/20 disabled:opacity-50"
        >
          <Save className="w-4 h-4 mr-2" />
          {isSaving ? 'Saving…' : 'Save changes'}
        </Button>
      </div>
    </Modal>
  );
};

export default EditUserModal;
