...
user_custom_restrictions = Table(
    ...
    Column(
        ...
        Integer,
        ForeignKey("custom_restrictions.id", ondelete="CASCADE"),
        primary_key=True,
    ),
)
...
class CustomRestriction(Base):
    ...
    )  # e.g., "NoWorkload", "NoAnalytics"
    tab_name = Column(
        String(100), nullable=False
    )  # e.g., "overview", "hub", "tracker", "calendar", "business", "goals", "activity", "pm"
    subsection = Column(
        String(100), nullable=False
    )  # e.g., "workload", "analytics", "calendar", "timeline", "goals"
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False, index=True)

    # Relationships
    users = relationship(
        "User", secondary=user_custom_restrictions, back_populates="custom_restrictions"
    )
