...
def seed(db):
    ...
    s1 = Sprint(id=100, project_id=1, name="Sprint 1", status="active",
                start_date=datetime(2026, 1, 1), end_date=datetime(2026, 1, 14))
    s2 = Sprint(id=101, project_id=2, name="Sprint 2", status="active",
                start_date=datetime(2026, 2, 1), end_date=datetime(2026, 2, 14))
    db.add_all([s1, s2])
    db.commit()

    epic = WorkItem(id=200, project_id=1, type="epic", title="Epic A",
                    status="in_progress", key="A-EPIC")
    parent = WorkItem(id=201, project_id=1, type="story", title="Story P",
                      status="todo", key="A-1")
    ...
    db.add_all([
        ...
        WorkItem(id=210, project_id=1, type="task", title="t1",
                 status="in_progress", key="A-2", assignee_id=10,
                 sprint_id=100, parent_id=201, epic_id=200,
                 story_points=3, estimated_hours=8),
        WorkItem(id=211, project_id=1, type="task", title="t2",
                 status="done", key="A-3", assignee_id=10,
                 sprint_id=100, parent_id=None, epic_id=200,
                 story_points=2, estimated_hours=4),
        WorkItem(id=212, project_id=2, type="task", title="t3",
                 status="todo", key="B-1", assignee_id=10,
                 sprint_id=101, parent_id=None, epic_id=None,
                 story_points=5, estimated_hours=12),
        # Not assigned to our developer
        WorkItem(id=213, project_id=1, type="task", title="t4-other",
                 status="todo", key="A-4", assignee_id=None,
                 sprint_id=100),
    ])
    db.commit()
...
class TestListWorkItems:
    ...
    def test_query_count_does_not_grow_with_item_count(self, db, seed):
        ...
        assert count["n"] <= 5, (
            f"list_work_items issued {count['n']} queries; expected ≤ 5"
        )
...
class TestMyTasks:
    ...
    def test_returns_empty_when_developer_has_no_account(self, db, seed):
        ghost = User(id=99, email="ghost@x.com", name="Ghost",
                     role="developer", is_active=True, is_first_login=False)
    ...
    def test_query_count_constant_regardless_of_item_count(self, db, seed):
        ...
        assert count["n"] <= 8, (
            f"my_tasks issued {count['n']} queries; expected ≤ 8"
        )
    ...
    def test_query_count_constant_with_more_items(self, db, seed):
        ...
        for i in range(300, 310):
            db.add(WorkItem(id=i, project_id=1, type="task",
                            title=f"extra-{i}", status="todo",
                            key=f"A-{i}", assignee_id=dev_id,
                            sprint_id=100))
        db.commit()
        ...
        assert count["n"] <= 8, (
            f"my_tasks issued {count['n']} queries for 13 items; "
            f"expected ≤ 8 (constant)"
        )
